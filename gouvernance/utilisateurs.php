...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['creer_compte'])) {
    ...
    if (!empty($nom) && !empty($email) && !empty($passwd) && $role_id > 0) {
        ...
        if ($verif->fetch()) {
            ...
            try {
                // 2. Hachage sécurisé du mot de passe (BCRYPT s'adapte parfaitement à ton VARCHAR(256))
                $password_hashed = password_hash($passwd, PASSWORD_BCRYPT);
                
                // 3. Insertion en base de données selon ta structure exacte
                ...
                $erreur = "Erreur lors de l'creation du compte : " . $e->getMessage();
            }
        }
    } else {
...
// ==========================================
// 2. RÉCUPÉRATION DES RÔLES POUR LE SÉLECTEUR
// ==========================================
$roles_disponibles = $pdo->query("SELECT id, nom_role FROM roles ORDER BY nom_role ASC")->fetchAll();

// ==========================================
// 3. CHARGEMENT DES COMPTES EXISTANTS AVEC LEUR RÔLE
// ==========================================
$sql_users = "SELECT u.*, r.nom_role  FROM utilisateurs u  INNER JOIN roles r ON u.role_id = r.id  ORDER BY r.nom_role ASC, u.nom ASC";
...
<div class="container mt-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted small mb-0">Gestion des accès au système, attribution des rôles et sécurité.</p>
        </div>
        
        <div>
            <a href="index.php" class="btn btn-outline-info btn-sm px-3 py-2 shadow-sm">
    ...
    <div class="row g-4">
            ...
            <div class="card border-0 shadow-sm sticky-top" style="top: 20px;">
                ...
                <div class="card-body pt-0">
                    <form method="POST" autocomplete="off">
                        ...
                        <button type="submit" name="creer_compte" class="btn btn-info w-100 shadow-sm">Générer les accès</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-8">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white border-0 py-3">
                    <h6 class="fw-bold mb-0"><i class="fa-solid fa-list text-secondary me-2"></i>Registre des accès actifs</h6>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-light small">
                                <tr>
                                    <th class="ps-3">Nom / Affichage</th>
                            ...
                            <tbody>
                                <?php if(empty($utilisateurs)): ?>
                                    <tr>
                                        <td colspan="4" class="text-center py-4 text-muted small">Aucun utilisateur enregistré.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach($utilisateurs as $user): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php 
                                                // Couleurs dynamiques selon le libellé du rôle
                                                $badge_color = "bg-secondary";
                                                if(strpos(strtolower($user['nom_role']), 'admin') !== false) $badge_color = "bg-danger";
                                                elseif(strpos(strtolower($user['nom_role']), 'trésorier') !== false || strpos(strtolower($user['nom_role']), 'tresorier') !== false) $badge_color = "bg-success";
                                                elseif(strpos(strtolower($user['nom_role']), 'pasteur') !== false) $badge_color = "bg-primary";
                                                elseif(strpos(strtolower($user['nom_role']), 'secrétaire') !== false || strpos(strtolower($user['nom_role']), 'secretaire') !== false) $badge_color = "bg-info text-dark";
                                                ?>
                                                <span class="badge <?= $badge_color ?>"><?= htmlspecialchars($user['nom_role']) ?></span>
                                            </td>
                                            <td>
